...
- Pipeline de IA: BiRefNet (Fal) → Normalización → Claude Haiku → **Gemini 2.0 Flash Preview Image Generation**
...
5. **Step 4:** Google Gemini `gemini-2.0-flash-preview-image-generation`
...
## Cambios aplicados (2026-03-23) — v22
- **Fix crítico:** Modelo `gemini-2.5-flash-preview-04-17` → `gemini-2.0-flash-preview-image-generation`
  - gemini-2.5-flash NO soporta generación de imágenes (solo texto)
  - El modelo dedicado para imagen es `gemini-2.0-flash-preview-image-generation`
- **responseModalities:** Cambiado de `['IMAGE']` a `['IMAGE', 'TEXT']` (requerido)
- Endpoint: `v1beta/models/gemini-2.0-flash-preview-image-generation:generateContent`

## Cambios anteriores (2026-03-23) — v21
- **Migración Step 4:** Fal.ai FLUX 2 Pro → Google Gemini (primer intento, modelo incorrecto)
...
- **`uploadFinalArt()`:** Ahora recibe `(base64, mimeType)` en vez de URL
...
- Monitorear logs step 4: tiempo de respuesta de Gemini
- Verificar calidad vs FLUX anterior
...
- **IMPORTANTE:** gemini-2.5-flash NO genera imágenes — usar `gemini-2.0-flash-preview-image-generation`